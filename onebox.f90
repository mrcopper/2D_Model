...
!set radial distance
!  rdist= 6   !in Rj
  rdist= 6.0+(3.0/(RAD_GRID-1))*(radgrid-1)   !in Rj
  print *, mype, "is at the radial distance", rdist, "Rj."
...
!set density values
  ...
  fehot_const= fehot_const*(rdist/6.0)**3.4
  trans = 4.62963e-7
...
!Improved Euler method
  ni%sp   = n%sp  + (nin%sp  - n%sp )*dep%transport*dt
  ni%s2p  = n%s2p + (nin%s2p - n%s2p)*dep%transport*dt 
  ni%s3p  = n%s3p + (nin%s3p - n%s3p)*dep%transport*dt 
  ni%op   = n%op  + (nin%op  - n%op )*dep%transport*dt 
  ni%o2p  = n%o2p + (nin%o2p - n%o2p)*dep%transport*dt 

  nrgi%sp   = nrg%sp   + (nTin%sp   - nrg%sp )*dep%transport*dt 
  nrgi%s2p  = nrg%s2p  + (nTin%s2p  - nrg%s2p)*dep%transport*dt 
  nrgi%s3p  = nrg%s3p  + (nTin%s3p  - nrg%s3p)*dep%transport*dt 
  nrgi%op   = nrg%op   + (nTin%op   - nrg%op )*dep%transport*dt 
  nrgi%o2p  = nrg%o2p  + (nTin%o2p  - nrg%o2p)*dep%transport*dt 
  ...
  n%sp   = n%sp  + .5 * (nin%sp  + ninold%sp - ni%sp - n%sp )* dep%transport*dt
  n%s2p  = n%s2p + .5 * (nin%s2p + ninold%s2p- ni%s2p- n%s2p)* dep%transport*dt
  n%s3p  = n%s3p + .5 * (nin%s3p + ninold%s3p- ni%s3p- n%s3p)* dep%transport*dt
  n%op   = n%op  + .5 * (nin%op  + ninold%op - ni%op - n%op )* dep%transport*dt
  n%o2p  = n%o2p + .5 * (nin%o2p + ninold%o2p- ni%o2p- n%o2p)* dep%transport*dt

  nrg%sp   = nrg%sp  + .5 * (nTin%sp  + nTinold%sp  - nrgi%sp - nrg%sp )* dep%transport*dt
  nrg%s2p  = nrg%s2p + .5 * (nTin%s2p + nTinold%s2p - nrgi%s2p- nrg%s2p)* dep%transport*dt
  nrg%s3p  = nrg%s3p + .5 * (nTin%s3p + nTinold%s3p - nrgi%s3p- nrg%s3p)* dep%transport*dt
  nrg%op   = nrg%op  + .5 * (nTin%op  + nTinold%op  - nrgi%op - nrg%op )* dep%transport*dt
  nrg%o2p  = nrg%o2p + .5 * (nTin%o2p + nTinold%o2p - nrgi%o2p- nrg%o2p)* dep%transport*dt
...
!!!!!!!!!!!!!!!!!!!!ENERGY!!!!!!!!!!!!!!!!!!!!!!
  if(outside<npes) call MPI_SEND(nrg%sp, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside.ge.0) call MPI_RECV(nTin%sp, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%s2p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside.ge.0) call MPI_RECV(nTin%s2p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%s3p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside.ge.0) call MPI_RECV(nTin%s3p, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%op, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
  if(inside.ge.0) call MPI_RECV(nTin%op, 1, MPI_DOUBLE_PRECISION, inside, 22, MPI_COMM_WORLD, stat, ierr)

  if(outside<npes) call MPI_SEND(nrg%o2p, 1, MPI_DOUBLE_PRECISION, outside, 22, MPI_COMM_WORLD, ierr)
